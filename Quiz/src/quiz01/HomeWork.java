		...
		x: while(true) {
			...
			case 2:
				...
				if(index <= 0) {
					...
					index--;
					System.out.println("다음 정보(이름)>"+nameList[index]+"\n");
					System.out.println("다음 정보(성별)>"+genderList[index]+"\n");
					System.out.println("다음 정보(나이)>"+ageList[index]+"\n");
				}
			...
			case 3:
				 ...
				 */
				if(index >= count-1) { //count가 2일때, 인덱스는 0에 있어야 다음이 있다
			...
			case 5:
				...
				if(index >= 0 && index < count) {
					...
					System.out.println("현재정보("+ genderList[index]+")>");
			...
			case 6:
				 ...
				 */
				if(index >= 0 && index < count) {
					for(int i = index; i < count -1; i++) { //count-1은 마지막 직전 인덱스
